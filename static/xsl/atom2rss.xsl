...
<x:template match="/atom:feed">
  <rss version="2.0">
    <channel>
      ...
      <!-- JJOR -->
      <atom:link
	      href="{substring-before(/atom:feed/atom:link[@rel='self']/@href, '.atom')}.rss"
...
<x:template name="rfc822Date">
  ...
  <x:value-of select="format-number(date:hour-in-day($isoDate), '00')"/>:<x:value-of select="format-number(date:minute-in-hour($isoDate), '00')"/>:<x:value-of select="format-number(date:second-in-minute($isoDate), '00')"/> <x:text> GMT</x:text>
</x:template>
