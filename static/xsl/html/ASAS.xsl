...
<!--
 * ASAS.xsl - templates específicas del test ASAS.
 *
-->
...
<xsl:transform
	...
	xmlns="&XHTMLNS;"
	xmlns:h="&XHTMLNS;"
	xmlns:p="urn:uuid:6869bf78-a7f1-11dd-b6a9-001fc6576244"
...
<xsl:template match="h:html" mode="test.includes">
	<script type="text/javascript" src="/scripts/ASAS">;</script>
</xsl:template>
...
<xsl:template match="sentencia" mode="likert.question-headers">
	<th class="OptionLabel" colspan="7">
		<span style="float: left;"><xsl:value-of select="tables:text('raro')"/></span>
		<span style="float: right;"><xsl:value-of select="tables:text('frecuente')"/></span>
	</th>
</xsl:template>
