...
<!--
 * uLS.xsl - templates específicas del test uLS.
 *
-->
...
<xsl:transform
	...
	xmlns="&XHTMLNS;"
	xmlns:h="&XHTMLNS;"
	xmlns:p="urn:uuid:687190ae-a7f1-11dd-b848-001fc6576244"
...
<xsl:template match="h:html" mode="test.includes">
	<script type="text/javascript" src="/scripts/ULS">;</script>
</xsl:template>
...
<xsl:template match="sentencia" mode="likert.question-headers">
	<th class="OptionLabel"><xsl:value-of select="tables:text('ulsN')"/></th>
	<th class="OptionLabel"><xsl:value-of select="tables:text('ulsR')"/></th>
	<th class="OptionLabel"><xsl:value-of select="tables:text('ulsS')"/></th>
	<th class="OptionLabel"><xsl:value-of select="tables:text('ulsO')"/></th>
</xsl:template>
